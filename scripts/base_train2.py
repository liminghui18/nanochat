"""
Train model using Flash Attention 2 (for A100/Ampere GPUs). From root directory of the project, run as:

python -m scripts.base_train2

or distributed as:

torchrun --nproc_per_node=8 -m scripts.base_train2

If you are only on CPU/Macbook, you'll want to train a much much smaller LLM. Example:
python -m scripts.base_train2 --depth=4 --max-seq-len=512 --device-batch-size=1 --eval-tokens=512 --core-metric-every=-1 --total-batch-size=512 --num-iterations=20
"""
...
from nanochat.flash_attention2 import HAS_FA2
from scripts.base_eval import evaluate_core
print_banner()

# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Pretrain base model with Flash Attention 2")
# Logging
...
from nanochat.flash_attention2 import USE_FA2
using_fa2 = USE_FA2
if using_fa2:
    print0("✓ Using Flash Attention 2 (Ampere GPU detected), efficient and optimized for A100.")
else:
    print0("!" * 80)
    if HAS_FA2 and COMPUTE_DTYPE not in (torch.bfloat16, torch.float16):
        print0(f"WARNING: Flash Attention 2 only supports bf16/fp16, but COMPUTE_DTYPE={COMPUTE_DTYPE}. Using PyTorch SDPA fallback")
    else:
        print0("WARNING: Flash Attention 2 not available, using PyTorch SDPA fallback")
    print0("WARNING: Training will be less efficient without FA2")
...
def build_model_meta(depth):
    ...
    # (FA2 requires head_dim divisible by 8, and this guarantees head_dim == args.head_dim exactly)
...
get_report().log(section="Base model training (FA2)", data=[
